...
class My_Model:
    def __init__(self, arg_input_shape, onto_shape, dimention):
        ...
        self.all_allgn = concatenate([self.all_allgn, self.onto_vec])
        ...
        self.model = Model(input=[self.arg_ip1, self.arg_ip2,self.onto_ip], output=self.prediction)
        sgd = SGD(lr = 0.1, momentum = 0.9, decay = 0, nesterov = False)
        self.model.compile(loss = 'mean_squared_error', optimizer = sgd, metrics = ['accuracy'])
        #seed(2017)
        #self.model.fit([X1, X2], Y.values, batch_size = 2000, nb_epoch = 100, verbose = 1)
    def train_model(self, arg_X1,  arg_X2, onto_ip, train_y,batch_size_=50,epch=15):
        self.model.fit([ arg_X1, arg_X2,onto_ip], train_y, batch_size=batch_size_, nb_epoch=epch, verbose=1, shuffle=True)
        self.epch = epch

    def predict(self,arg_x1, arg_x2, onto_ip ):
        return self.model.predict([arg_x1,arg_x2,onto_ip])

    def evaluate(self,A1, A2, ontoip, y):
        return self.model.evaluate([A1,A2,ontoip,y])
    ...
    def save_model(self, path):
        model_file = os.path.join(path,'model_only_onto_'+str(self.epch)+'.h5')

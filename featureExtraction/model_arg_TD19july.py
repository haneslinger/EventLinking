...
class My_Model:
    def __init__(self,ev_input_shape, arg_input_shape, dimention):
        ...
        self.projection2 = TimeDistributed(Dense(8,activation='relu'), input_shape=(self.sh_projection1_op1.shape[1], self.sh_projection1_op1.shape[2]))#(self.merged_layer
        ...
        self.ev_projection = Dense(8,activation='relu')
        ...
        self.all_allgn = concatenate([self.all_allgn, self.ev_score])
        ...
        self.model = Model(input=[self.ev_ip1, self.arg_ip1, self.ev_ip2, self.arg_ip2], output=self.prediction)
        sgd = SGD(lr = 0.1, momentum = 0.9, decay = 0, nesterov = False)
        self.model.compile(loss = 'mean_squared_error', optimizer = sgd, metrics = ['accuracy'])
        #seed(2017)
        #self.model.fit([X1, X2], Y.values, batch_size = 2000, nb_epoch = 100, verbose = 1)
    def train_model(self,train_X1, arg_X1, train_X2, arg_X2, train_y,batch_size_=50,epch=15):
        self.model.fit([train_X1, arg_X1, train_X2, arg_X2], train_y, batch_size=batch_size_, nb_epoch=epch, verbose=1, shuffle=True)
        self.epch = epch

    def predict(self,test_x1,arg_x1,test_x2, arg_x2):
        return self.model.predict([test_x1, arg_x1, test_x2, arg_x2])

    def evaluate(self,X1,A1, X2, A2, y):
        return self.model.evaluate([X1,A1,X2,A2],y)
    ...
    def save_model(self, path):
        model_file = os.path.join(path,'model_arg_TD_'+str(self.epch)+'.h5')
